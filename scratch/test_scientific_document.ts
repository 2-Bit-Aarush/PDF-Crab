...
const scientificModel: NotebookViewModel = {
...
async function runTests() {
  ...
  const mdText = await mdAdapter.transform(scientificModel);
  ...
  const docxHtml = await docxAdapter.transform(scientificModel);
  ...
  try {
    ...
    const pdfBuffer = await pdfAdapter.transform(scientificModel);
